...
void changeRoll(rotation *degTravelled, duty *MaxMin) {
      ...
      //if expected new duty cycle exceeds the maximum duty cycle then return the max
      if (((float)MaxMin->current - degTravelled->roll * MaxMin->factor) >= MaxMin->max) {
          MaxMin->current = (int)MaxMin->max; 
      
      } else if (((float)MaxMin->current - degTravelled->roll *  MaxMin->factor) <= MaxMin->min) {
          MaxMin->current = (int)MaxMin->min;
   
      } else {
          MaxMin->current = (int)((float)MaxMin->current - degTravelled->roll *  MaxMin->factor);
