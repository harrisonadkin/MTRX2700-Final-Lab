...
void calibrate_gryo(GyroRaw *raw_data, GyroOffsets *scaled_data)
{
   x_offset = 0 - (float)(raw_data->x);
   y_offset = 0 - (float)(raw_data->y);
   z_offset = 0 - (float)(raw_data->z);
  
  scaled_data->x = x_offset;  
  scaled_data->y = y_offset; 
  scaled_data->z = z_offset; 



}

void cleaned_gryo(GyroRaw *raw_data, GyroOffsets *scaled_data)
{
  raw_data->x = raw_data->x + scaled_data->x;
  raw_data->y = raw_data->y + scaled_data->y;
  raw_data->z = raw_data->z + scaled_data->z;
